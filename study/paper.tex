...
matches and reproduces that score in two independent runs; a
transcript-only Scribe baseline with no audio LLM and no re-listen
reaches 26 of 30. The audio LLM by itself
...
this matches what the standard \verb|python -m study.cli| run
...
harness. Numbers reported are best-of-n; the dual-STT 29 was
...
\midrule
Voxtral+Scribe \,|\, AL+ \,|\, T+ \,|\, W+ & 3 & \textbf{29} & full pipeline; the headline; reproduced in 2 of 3 runs \\
...
reaches 29 of 30 and reproduced that score in two independent runs.
The chart plots the best score achieved by each configuration, since
that is what the standard \verb|python -m study.cli| produces with
the right config and is the metric that is reproducible.
...
runs I recorded; the dual-STT cell hit 29 in two of three runs. The
label encodes the four design axes
...
gain on the dual-STT cell, which reproduced 29 of 30 in two
independent runs. There is run-to-run variance in this cell because
the reconciler now has more candidates to disagree among, and the
candidate-1 LLM votes are sampled at temperature 0.7; one of the
three dual-STT runs landed at 24 instead of 29 due to an unlucky
tie-break on a borderline candidate.
