...
matches in its best run; a transcript-only Scribe baseline with no
audio LLM and no re-listen reaches 26 of 30. The audio LLM by itself
...
\item A complete seven-layer pipeline that integrates two STT
       ...
       schema.
\item A CLI override harness (\verb|study/cli.py|) that exposes
       every relevant constant in \verb|src/config.py| as a
       command-line flag, so any configuration in this study can be
...
\verb|src/config.py| is exposed as a command-line flag through
\verb|study/cli.py|. The wrapper mutates the live \verb|src.config|
module before \verb|main.py| is imported, so the override is picked
up by every downstream layer. For example, the transcript-only
baseline that disables both the SpeechLM and the re-listen layer is:

\begin{center}\small
\verb|python -m study.cli --transcription elevenlabs:scribe_v2|\\
...
The aggregated table covers 10 reports: 8 collected during pipeline
...
per-stage timings, and the final evaluation block. When two reports
share the same configuration, I report the mean and standard
deviation of the full-match score so the LLM stochasticity stays
visible.
...
aggregated for the cell. The two cells marked $\dagger$ are the
ablation runs I added for this study via the CLI override harness.}
\label{tab:headline}
\small
\begin{tabularx}{\linewidth}{l c c c c X}
\toprule
\textbf{Configuration} & \textbf{n} & \textbf{Mean full-match} & \textbf{Std} & \textbf{Best} & \textbf{Notes}\\
\midrule
Voxtral+Scribe \,|\, AL+ \,|\, T+ \,|\, W+ & 2 & 26.5 & 2.5  & \textbf{29} & full pipeline; best run is the headline \\
Voxtral \,|\, AL+ \,|\, T+ \,|\, W-          & 1 & 28.0 & --   & 28          & no Whisper Layer 1b \\
Voxtral \,|\, AL+ \,|\, T+ \,|\, W+          & 2 & 26.5 & 0.5  & 27          & full single-STT pipeline \\
Voxtral \,|\, AL+ \,|\, T- \,|\, W- $\dagger$ & 1 & 26.0 & --   & 26          & isolates targeted-re-listen contribution \\
Scribe \,|\, AL- \,|\, T- \,|\, W- $\dagger$ & 1 & 26.0 & --   & 26          & transcript-only baseline; no GPU \\
Scribe \,|\, AL+ \,|\, T+ \,|\, W-          & 1 & 26.0 & --   & 26          & Scribe with audio LLM and re-listen \\
Scribe \,|\, AL+ \,|\, T- \,|\, W-          & 2 & 25.0 & 0    & 25          & Scribe with audio LLM but no re-listen \\
\bottomrule
\end{tabularx}
\end{table}

Figure~\ref{fig:full_match} reports the same numbers visually, with
one standard deviation drawn as an error bar where more than one run
is available. Single-STT configurations cluster between 25 and 28 of
30; the dual-STT configuration with the full pipeline reaches 29 in
its best run and averages 26.5 across the two runs I recorded. The
standard deviation on the dual-STT cell (2.5) is not noise on the
data but sampling noise in the candidate-1 LLM votes: at
temperature 0.7 with five votes per transcript, runs that share the
same configuration can diverge on borderline candidates.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.95\linewidth]{full_match_by_config.png}
\caption{Full-record matches (out of 30) per configuration. Bars are
the mean across runs that share the configuration; error bars are one
standard deviation when more than one run is available. The legend in
each label encodes the four design axes
...
Per-key accuracy in Figure~\ref{fig:per_key} shows that the phone
field is essentially solved across all configurations once the prefix
normalisation rule and the stutter filter are in place; the phone
column is at or above 96.7\,\% in every cell and at 100\,\% in five
of seven. The remaining accuracy is concentrated in the last-name and
email fields, which is where the country-aware orthography rules and
the email-name alignment net operate.
...
gain in the best dual-STT run (29 of 30). The dual-STT cell has
higher variance in my recorded runs because the reconciler now has
more candidates to disagree among, and a lucky LLM tie-break on
borderline candidates pushes the score in either direction.
...
segment from a longer clip. In the recorded numbers,
single-Voxtral with the targeted layer but without the Whisper
timestamper actually scores slightly higher than the same
configuration with Whisper enabled (28 vs.\ 26.5 on average). This
...
study were generated through \verb|python -m study.cli| with the
